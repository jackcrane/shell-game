...
export const startServer = async () => {
  ...
  const gameNames = discoverGameNames();
  const exampleGame = gameNames.includes("tic-tac-toe")
    ? "tic-tac-toe"
    : gameNames[0] ?? null;

  console.log(`Available games: ${gameNames.join(", ") || "(none)"}`);
  console.log(
    exampleGame
      ? `Example: ${getConnectExample(exampleGame)}`
      : "Example: add a game under ./games to connect.",
  );
